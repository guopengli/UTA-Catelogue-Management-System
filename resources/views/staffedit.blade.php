...
<div style="height: 600px">
	<div class="signin">
	  <form class="adduser" action= "/staffedit/{{$id->uid}}/{{$course->cid}}" method="post">
	  	@csrf
	  	<ul>
	    ...
	    <label for="cname"><b>Course Number</b></label>
	    <input type="text" name="cnum" value="{{$course->cnum}}">
	    <label for="ctitle"><b>Course Title</b></label>
		<input type="text" name="ctitle" value="{{$course->ctitle}}">
		<label for="ctype"><b>Course Type</b></label>
		<select name="ctype">
				    <option value="u" {{$course->ctype =='u' ? 'selected': ''}}>Upcoming</option>
				    <option value="p" {{$course->ctype =='p' ? 'selected': ''}}>Planned</option>
				</select> 
